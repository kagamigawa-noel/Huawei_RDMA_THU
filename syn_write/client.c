...
int clean_task( struct task_active *now );
...
int clean_task( struct task_active *now )
{
	...
	reback = update_bit( memgt->peer_bit, RDMA_BUFFER_SIZE/request_size/thread_number*t_pos,\
		RDMA_BUFFER_SIZE/request_size/thread_number, data, 1 );
	if( reback != 0 ){
